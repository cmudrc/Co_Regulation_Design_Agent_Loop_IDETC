...
_DESIGN_INSTRUCTIONS = """\
    ...
    IMPORTANT: After each step, you will receive feedback from the battery pack simulator showing the actual performance results of your designs. Use this feedback to guide your next design iteration.

    TERMINATION RULES:
    - Only output <answer> AFTER you have received feedback from the simulator and the design is valid (Design status: FEASIBLE).
    ...
    Guidance: 
    Think about these questions before starting your design. 
    - "What is my goal for this design problem?" 
    - "What criteria will I use to judge success?"
    - "What is my plan?"
    - "What could go wrong and what is my fallback?"

    You need to monitor your optimization trajectory — track progress trends, detect stalls or regressions, and consider strategic shifts.
    You may be forgetful, so you need to remind yourself of the design objectives and your progress.
    You may also be fixated, so you need to remind yourself to explore the design space, try potential alternatives, and pursuit higher performance designs. 
    """
...
def design_agent_follow_up_message(step, step_history):
    ...
    feedback_prompt += analyze_progress(step_history)
...
RECOMMENDED_ACTION: [one specific strategic suggestion for your next iteration]
"""
