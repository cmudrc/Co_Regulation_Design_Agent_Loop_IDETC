...
from design_problem_infra.tracker import step_conversion, record_crdal_response, DesignStep, DesignTrace
from design_problem_infra.validation import validate_design, print_validation_summary

from message.message_CRDAL import (
    design_agent_initial_message,
    design_agent_follow_up_message,
    CRDAL_agent_initial_message,
    CRDAL_agent_follow_up_message
    )

# ==================== USER CONFIGURATION ====================
# Model configuration
MODEL_KEY_DESIGN_AGENT = "gemini-3.1-pro-preview"
MODEL_KEY_CRDAL_AGENT = "gemini-3.1-pro-preview"
# ============================================================
...
def main(problem_query=None, 
    ...
    crdal_agent_temperature = 1.0
    design_agent_max_tokens = 65536
    crdal_agent_max_tokens = 65536

    DesignAgent = GoogleGeminiAdapter(model_name=MODEL_KEY_DESIGN_AGENT, thinking_level="high", seed=model_seed)
    CRDALAgent = GoogleGeminiAdapter(model_name=MODEL_KEY_CRDAL_AGENT, thinking_level="high", seed=model_seed)

    design_agent_messages = []
    CRDAL_agent_messages = []
    step_history = []
    is_valid = False
    is_done = False
    ...
    design_tree.metadata["system_version"] = "CRDAL"
    ...
    design_tree.metadata["crdal_agent"] = {
        "model": CRDALAgent.model_name,
        "thinking_level": CRDALAgent.thinking_level,
        "seed": CRDALAgent.seed,
        "temperature": crdal_agent_temperature,
    }
    ...
    CRDAL_system_prompt = """
    You are a design process monitor. Your job is to observe the designer's optimization trajectory and provide strategic guidance.

    You do NOT give domain-specific engineering advice (e.g., how battery cells work, what voltage means, how series/parallel affects capacity).
    You DO identify when the designer is stuck, regressing, or making good progress, and suggest high-level strategy shifts (e.g., "try a fundamentally different layout", "the current approach is working. keep refining", "revert to the previous design").

    Always respond in the structured format requested.
    """
    initial_feedback_prompt = CRDAL_agent_initial_message(problem_query)
    CRDAL_initial_msg = [
        {"role": "system", "content": CRDAL_system_prompt},
        {"role": "user", "content": initial_feedback_prompt}
        ]
    CRDAL_agent_messages.extend(CRDAL_initial_msg)
    ...
    while (is_valid == False or is_done == False or step_count < min_steps) and step_count < max_steps:
        ...
        if step_count >= min_steps and is_valid and is_done: # if success, no CRDAL
            # update step
            ...
            # update CRDAL agent message with trajectory history
            CRDAL_msg_content = CRDAL_agent_follow_up_message(step, step_history)
            CRDAL_agent_messages.append({"role": "user", "content": CRDAL_msg_content})
            # CRDAL agent generate "assistant" response
            CRDAL_response = CRDALAgent.generate(CRDAL_agent_messages, temperature=crdal_agent_temperature, max_tokens=crdal_agent_max_tokens)
            CRDAL_response_text = CRDAL_response.text
            CRDAL_agent_messages.append({"role": "assistant", "content": CRDAL_response_text})

            # store feedback and token usage in step and add to history
            record_crdal_response(step, CRDAL_response)
            design_tree.add_step(step)
            step_history.append(step)

            # update design agent message
            # generate "user" message to design agent
            design_msg_content = design_agent_follow_up_message(CRDAL_response_text, step)
    ...
    design_tree.add_message_history(CRDAL_agent_messages)
...
def batch_experiments(num_repeats=30, 
    ...
    if resume_dir:
        ...
        basename = generate_timestamped_basename()
        batch_dir = os.path.join(Path(__file__).parent.parent.parent, "output", f"CRDAL_RE_G31P_750_{basename}")
